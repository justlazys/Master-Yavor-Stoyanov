<template>
    <div>
        <div class="row">
            ...
            <div class="mt-3">
                <button class="btn btn-sm btn-primary  " @click="addGrade()">Добави</button>
                <button class="btn btn-sm btn-primary ml-10 " @click="back()">Назад</button>
            </div>
        </div>
            ...
            <vue-good-table v-if="rows.length != 0" :columns="columns" :rows="rows" :pagination-options="{
                ...
                <template #table-row="props">
                    ...
                    <span v-else-if="props.column.field === 'control'">
                        <div>
                            <p>
                                {{ translateDisc(props.row.disc,user.faculty) }} Тип: {{ this.translateType(props.row.type) }}, Оценка:{{ props.row.note }}  от {{props.row.fromUser  }} на {{ this.formatDateTime(props.row.date) }}  семестър {{ props.row.semester }}
                            </p>
                            </div>
                    </span>
...
export default {
    ...
    methods: {
        ...
        validation(data: GradeModel) {
            ...
            if (data.lecUser) {
                if (this.isString(data.lecUser)) {
                    msgs.push("Полето 'Преподавател на Лекции' приема само цифри");
                }
            }
            if (data.labUser) {
                if (this.isString(data.labUser)) {
                    msgs.push("Полето 'Преподавател на Лаб. упр' приема само цифри");
                }
            }
            if (data.labUser) {
                if (this.isString(data.labUser)) {
                    msgs.push("Полето 'Преподавател на Сем.упр.' приема само цифри");
                }
            }
            return msgs;
        },
        async addGrade() {
            ...
            this.grade.userEmail=this.$store.getters.retrieveEmail
